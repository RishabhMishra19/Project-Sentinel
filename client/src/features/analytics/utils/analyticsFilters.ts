 ...
 */
export const isoExclusiveToToDateInput = (
...
export const dateToFromIso = (date: string): string =>
...
export const filtersFromSearchParams = (
  ...
  filters.bucket =
    parseBucket(params.get('bucket')) ?? suggestedBucket(fromIso, toIso)
...
export const mapAnalyticsFiltersToPatch = (
  ...
  let productId =
    scope === 'PRODUCT' ? (apiFilters.productId ?? null) : null
  let serviceId =
    scope === 'SERVICE' || scope === 'ENDPOINT'
      ? (apiFilters.serviceId ?? null)
      : null
  let endpointId =
    scope === 'ENDPOINT' ? (apiFilters.endpointId ?? null) : null
  ...
  const bucket =
    parseBucket(apiFilters.bucket ?? null) ?? suggestedBucket(fromIso, toIso)
