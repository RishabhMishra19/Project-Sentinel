...
import {
  ...
  createRequestLogRowActions,
} from './requestLogsTableConfig'
...
export function RequestLogsTable({
  ...
  const productId =
    typeof fetchQuery?.filters.productId === 'string'
      ? fetchQuery.filters.productId
      : undefined
  const serviceId =
    typeof fetchQuery?.filters.serviceId === 'string'
      ? fetchQuery.filters.serviceId
      : undefined
  ...
  const serviceOptions = useMemo(() => {
    const scoped = productId
      ? services.filter((s) => s.productId === productId)
      : services
    return scoped.map((s) => ({
  ...
  const rowActions = useMemo(
    () => createRequestLogRowActions({ onView }),
    [onView],
  )
