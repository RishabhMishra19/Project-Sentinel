...
import {
  apiKeyColumns,
  createApiKeyRowActions,
} from './apiKeysTableConfig'
...
export const ApiKeysTable = ({
  ...
  const { data, isFetching } = useServiceApiKeysQuery(
    productId,
    serviceId,
    listParams,
  )
  ...
  const rowActions = useMemo(
    () => createApiKeyRowActions({ onRevoke }),
    [onRevoke],
  )
  ...
  const { tableProps } = useServerDataTable({
    ...
    toolbarActions: (
      <button
        ...
        title={
          hasActiveKey
            ? 'Revoke the active key before creating a new one'
            : undefined
        }
      >
        Create API key
