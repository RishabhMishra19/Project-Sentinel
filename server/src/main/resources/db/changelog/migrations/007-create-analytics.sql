...
--   *_stats_minute                 → 30 days
--   *_stats_hour                   → 1 year
--   *_stats_day                    → forever
--   endpoint_status_metrics        → 30 days (minute buckets)
--   endpoint_exception_metrics     → 30 days (minute buckets)
...
CREATE INDEX idx_endpoint_stats_minute_bucket ON endpoint_stats_minute (bucket_start DESC);
...
CREATE INDEX idx_endpoint_stats_hour_bucket ON endpoint_stats_hour (bucket_start DESC);
...
CREATE INDEX idx_endpoint_stats_day_bucket ON endpoint_stats_day (bucket_start DESC);
...
CREATE INDEX idx_service_stats_minute_bucket ON service_stats_minute (bucket_start DESC);
...
CREATE INDEX idx_service_stats_hour_bucket ON service_stats_hour (bucket_start DESC);
...
CREATE INDEX idx_service_stats_day_bucket ON service_stats_day (bucket_start DESC);
...
CREATE INDEX idx_product_stats_minute_bucket ON product_stats_minute (bucket_start DESC);
...
CREATE INDEX idx_product_stats_hour_bucket ON product_stats_hour (bucket_start DESC);
...
CREATE INDEX idx_product_stats_day_bucket ON product_stats_day (bucket_start DESC);

-- changeset sentinel:007-analytics-tenant-stats
CREATE TABLE tenant_stats_minute (
    bucket_start TIMESTAMP WITH TIME ZONE NOT NULL,
    ...
    request_bytes_total BIGINT NOT NULL,
    response_bytes_total BIGINT NOT NULL,
    CONSTRAINT pk_tenant_stats_minute PRIMARY KEY (bucket_start, tenant_id),
    CONSTRAINT fk_tenant_stats_minute_tenant FOREIGN KEY (tenant_id) REFERENCES tenants (id) ON DELETE RESTRICT
);

CREATE INDEX idx_tenant_stats_minute_tenant_bucket ON tenant_stats_minute (tenant_id, bucket_start DESC);
CREATE INDEX idx_tenant_stats_minute_bucket ON tenant_stats_minute (bucket_start DESC);

CREATE TABLE tenant_stats_hour (
    bucket_start TIMESTAMP WITH TIME ZONE NOT NULL,
    tenant_id UUID NOT NULL,
    request_count BIGINT NOT NULL,
    error_count BIGINT NOT NULL,
    ...
    request_bytes_total BIGINT NOT NULL,
    response_bytes_total BIGINT NOT NULL,
    CONSTRAINT pk_tenant_stats_hour PRIMARY KEY (bucket_start, tenant_id),
    CONSTRAINT fk_tenant_stats_hour_tenant FOREIGN KEY (tenant_id) REFERENCES tenants (id) ON DELETE RESTRICT
);

CREATE INDEX idx_tenant_stats_hour_tenant_bucket ON tenant_stats_hour (tenant_id, bucket_start DESC);
CREATE INDEX idx_tenant_stats_hour_bucket ON tenant_stats_hour (bucket_start DESC);

CREATE TABLE tenant_stats_day (
    bucket_start TIMESTAMP WITH TIME ZONE NOT NULL,
    tenant_id UUID NOT NULL,
    request_count BIGINT NOT NULL,
    error_count BIGINT NOT NULL,
    ...
    request_bytes_total BIGINT NOT NULL,
    response_bytes_total BIGINT NOT NULL,
    CONSTRAINT pk_tenant_stats_day PRIMARY KEY (bucket_start, tenant_id),
    CONSTRAINT fk_tenant_stats_day_tenant FOREIGN KEY (tenant_id) REFERENCES tenants (id) ON DELETE RESTRICT
);

CREATE INDEX idx_tenant_stats_day_tenant_bucket ON tenant_stats_day (tenant_id, bucket_start DESC);
CREATE INDEX idx_tenant_stats_day_bucket ON tenant_stats_day (bucket_start DESC);

-- changeset sentinel:007-analytics-endpoint-status-metrics
-- Exact HTTP status distribution per endpoint (minute buckets)
CREATE TABLE endpoint_status_metrics (
    bucket_start TIMESTAMP WITH TIME ZONE NOT NULL,
    endpoint_id UUID NOT NULL,
    status_code INTEGER NOT NULL,
    request_count BIGINT NOT NULL,
    CONSTRAINT pk_endpoint_status_metrics PRIMARY KEY (bucket_start, endpoint_id, status_code),
    CONSTRAINT fk_endpoint_status_metrics_endpoint FOREIGN KEY (endpoint_id) REFERENCES endpoints (id) ON DELETE RESTRICT
);

CREATE INDEX idx_endpoint_status_metrics_endpoint_bucket ON endpoint_status_metrics (endpoint_id, bucket_start DESC);
CREATE INDEX idx_endpoint_status_metrics_bucket ON endpoint_status_metrics (bucket_start DESC);

-- changeset sentinel:007-analytics-endpoint-exception-metrics
-- Aggregated exception counts per endpoint (minute buckets)
CREATE TABLE endpoint_exception_metrics (
    bucket_start TIMESTAMP WITH TIME ZONE NOT NULL,
    endpoint_id UUID NOT NULL,
    exception_type VARCHAR(256) NOT NULL,
    exception_count BIGINT NOT NULL,
    CONSTRAINT pk_endpoint_exception_metrics PRIMARY KEY (bucket_start, endpoint_id, exception_type),
    CONSTRAINT fk_endpoint_exception_metrics_endpoint FOREIGN KEY (endpoint_id) REFERENCES endpoints (id) ON DELETE RESTRICT
);

CREATE INDEX idx_endpoint_exception_metrics_endpoint_bucket ON endpoint_exception_metrics (endpoint_id, bucket_start DESC);
CREATE INDEX idx_endpoint_exception_metrics_bucket ON endpoint_exception_metrics (bucket_start DESC);
