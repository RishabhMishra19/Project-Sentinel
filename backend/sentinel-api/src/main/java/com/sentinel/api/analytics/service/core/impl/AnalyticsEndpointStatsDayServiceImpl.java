...
import com.sentinel.common.analytics.endpoint.repository.AnalyticsEndpointStatsDayRepository;
...
public class AnalyticsEndpointStatsDayServiceImpl implements AnalyticsService {

    private final AnalyticsEndpointStatsDayRepository EndpointStatsDayRepository;

    @Override
    public AnalyticsMetricsAggregate summary(UUID EndpointId, Instant from, Instant to, AnalyticsBucket bucket) {
        List<AnalyticsEndpointStatsDay> statsList = EndpointStatsDayRepository.findByEndpointIdAndBucketStartBetween(
                EndpointId,
                from,
                to);
    ...
    private void accumulate(AnalyticsMetricsAggregate aggregate, List<AnalyticsEndpointStatsDay> statsList) {
        long latencyP50 = 0;
        long latencyP95 = 0;
        long latencyP99 = 0;
        for (AnalyticsEndpointStatsDay stats : statsList) {
