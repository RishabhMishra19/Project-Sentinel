...
import com.sentinel.common.analytics.service.repository.AnalyticsServiceStatsDayRepository;
...
public class AnalyticsServiceStatsDayServiceImpl implements AnalyticsService {

    private final AnalyticsServiceStatsDayRepository ServiceStatsDayRepository;

    @Override
    public AnalyticsMetricsAggregate summary(UUID ServiceId, Instant from, Instant to, AnalyticsBucket bucket) {
        List<AnalyticsServiceStatsDay> statsList = ServiceStatsDayRepository.findByServiceIdAndBucketStartBetween(
                ServiceId,
                from,
                to);
    ...
    private void accumulate(AnalyticsMetricsAggregate aggregate, List<AnalyticsServiceStatsDay> statsList) {
        long latencyP50 = 0;
        long latencyP95 = 0;
        long latencyP99 = 0;
        for (AnalyticsServiceStatsDay stats : statsList) {
