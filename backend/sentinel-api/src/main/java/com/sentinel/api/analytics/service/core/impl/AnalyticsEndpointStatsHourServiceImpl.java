...
import com.sentinel.common.analytics.endpoint.repository.AnalyticsEndpointStatsHourRepository;
...
public class AnalyticsEndpointStatsHourServiceImpl implements AnalyticsService {

    private final AnalyticsEndpointStatsHourRepository EndpointStatsHourRepository;

    @Override
    public AnalyticsMetricsAggregate summary(UUID EndpointId, Instant from, Instant to, AnalyticsBucket bucket) {
        List<AnalyticsEndpointStatsHour> statsList = EndpointStatsHourRepository.findByEndpointIdAndBucketStartBetween(
                EndpointId,
                from,
                to);
    ...
    private void accumulate(AnalyticsMetricsAggregate aggregate, List<AnalyticsEndpointStatsHour> statsList) {
        long latencyP50 = 0;
        long latencyP95 = 0;
        long latencyP99 = 0;
        for (AnalyticsEndpointStatsHour stats : statsList) {
