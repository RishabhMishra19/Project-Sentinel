...
import com.sentinel.common.analytics.tenant.repository.AnalyticsTenantStatsMinuteRepository;
...
public class AnalyticsTenantStatsMinuteServiceImpl implements AnalyticsService {

    private final AnalyticsTenantStatsMinuteRepository TenantStatsMinuteRepository;

    @Override
    public AnalyticsMetricsAggregate summary(UUID TenantId, Instant from, Instant to, AnalyticsBucket bucket) {
        List<AnalyticsTenantStatsMinute> statsList = TenantStatsMinuteRepository.findByTenantIdAndBucketStartBetween(
                TenantId,
                from,
                to);
    ...
    private void accumulate(AnalyticsMetricsAggregate aggregate, List<AnalyticsTenantStatsMinute> statsList) {
        long latencyP50 = 0;
        long latencyP95 = 0;
        long latencyP99 = 0;
        for (AnalyticsTenantStatsMinute stats : statsList) {
