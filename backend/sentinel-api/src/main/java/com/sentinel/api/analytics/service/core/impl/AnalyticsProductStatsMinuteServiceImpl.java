...
import com.sentinel.common.analytics.product.repository.AnalyticsProductStatsMinuteRepository;
...
public class AnalyticsProductStatsMinuteServiceImpl implements AnalyticsService {

    private final AnalyticsProductStatsMinuteRepository ProductStatsMinuteRepository;

    @Override
    public AnalyticsMetricsAggregate summary(UUID ProductId, Instant from, Instant to, AnalyticsBucket bucket) {
        List<AnalyticsProductStatsMinute> statsList = ProductStatsMinuteRepository.findByProductIdAndBucketStartBetween(
                ProductId,
                from,
                to);
    ...
    private void accumulate(AnalyticsMetricsAggregate aggregate, List<AnalyticsProductStatsMinute> statsList) {
        long latencyP50 = 0;
        long latencyP95 = 0;
        long latencyP99 = 0;
        for (AnalyticsProductStatsMinute stats : statsList) {
