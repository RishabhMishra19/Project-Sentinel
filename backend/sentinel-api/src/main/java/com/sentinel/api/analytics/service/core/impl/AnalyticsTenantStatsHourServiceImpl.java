...
import com.sentinel.common.analytics.tenant.repository.AnalyticsTenantStatsHourRepository;
...
public class AnalyticsTenantStatsHourServiceImpl implements AnalyticsService {

    private final AnalyticsTenantStatsHourRepository TenantStatsHourRepository;

    @Override
    public AnalyticsMetricsAggregate summary(UUID TenantId, Instant from, Instant to, AnalyticsBucket bucket) {
        List<AnalyticsTenantStatsHour> statsList = TenantStatsHourRepository.findByTenantIdAndBucketStartBetween(
                TenantId,
                from,
                to);
    ...
    private void accumulate(AnalyticsMetricsAggregate aggregate, List<AnalyticsTenantStatsHour> statsList) {
        long latencyP50 = 0;
        long latencyP95 = 0;
        long latencyP99 = 0;
        for (AnalyticsTenantStatsHour stats : statsList) {
