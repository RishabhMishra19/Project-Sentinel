...
public class KafkaTopologyController {
    ...
    private static final String CONSUMER_GROUP_ID = "sentinel-processor-group";
    ...
    public Map<String, Object> getTopology() throws Exception {
        ...
        try (AdminClient client = AdminClient.create(kafkaAdmin.getConfigurationProperties())) {
            ...
            Collection<TopicListing> topics = client.listTopics().listings().get();
            ...
            Map<String, org.apache.kafka.clients.admin.TopicDescription> descriptions = client.describeTopics(activeBrokerTopics).allTopicNames().get();
            ...
            if (!partitions.isEmpty()) {
                ...
                Map<TopicPartition, ListOffsetsResult.ListOffsetsResultInfo> endOffsets = client.listOffsets(request).all().get();

                Map<TopicPartition, Long> endOffsetMap = new HashMap<>();
                for (var entry : endOffsets.entrySet()) {
                    endOffsetMap.put(entry.getKey(), entry.getValue().offset());
                    String topic = entry.getKey().topic();
                    topicOffsets.put(topic, topicOffsets.getOrDefault(topic, 0L) + entry.getValue().offset());
                }

                // 2. Fetch consumer group committed offsets to compute actual lag
                try {
                    Map<String, ConsumerGroupDescription> groupDesc = client.describeConsumerGroups(List.of(CONSUMER_GROUP_ID)).all().get();
                    if (groupDesc.containsKey(CONSUMER_GROUP_ID)) {
                        Map<TopicPartition, OffsetAndMetadata> committedOffsets = client.listConsumerGroupOffsets(CONSUMER_GROUP_ID)
            ...
            for (org.apache.kafka.streams.TopologyDescription.Subtopology subtopology : description.subtopologies()) {
                for (org.apache.kafka.streams.TopologyDescription.Node node : subtopology.nodes()) {
                    if (node instanceof org.apache.kafka.streams.TopologyDescription.Source) {
                        org.apache.kafka.streams.TopologyDescription.Source sourceNode =
                                (org.apache.kafka.streams.TopologyDescription.Source) node;

                        for (String topicName : sourceNode.topicSet()) {
                            if (topicName.startsWith("__") || topicName.contains("-changelog") || topicName.contains("-repartition")) {
                                continue;
                            }
